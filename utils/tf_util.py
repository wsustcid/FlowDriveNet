...
Updated by Wang Shuai 

TODO:
Change the indentattion to 4
"""
...
def model_summary():
  """ Summary the model structure and Params
	"""
...
def Batch_Normalization(X, is_training, scope):
  ...
  Hint:
    ...
    - reuse parameter is designed to reuse existing variables Sharing Variables in between different ops in the graph. During a neural network training step parameters are learned; so to use those learned parameters during validation you can retrieve the variables using reuse parameters without resetting the values or without recreating those variables.
    Refer:
...
def lstm(inputs, hidden_size, scope):
  """ input (B,T,D)
  return: (B,H)
  """
  with tf.variable_scope(scope) as sc:
    lstm_cell = tf.contrib.rnn.LSTMCell(hidden_size, forget_bias=1.0)
    outputs, state = tf.nn.dynamic_rnn(lstm_cell, inputs, dtype=tf.float32) # (B,T,H)
